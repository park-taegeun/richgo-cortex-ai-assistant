...
from src.analytics.temporal import PIRBandAnalyzer, REGIONAL_INCOME_MAN_WON
...
ADAPTIVE_JEONSE_FLOOR: dict = {
    "강남구": 0.35, "용산구": 0.35, "성동구": 0.35,
    "서초구": 0.38, "송파구": 0.41,
    "마포구": 0.48, "종로구": 0.48,
    "서울_기타": 0.55, "경기": 0.65, "default": 0.65,
}
...
W_SUPPLY, W_PIR, W_JEONSE, W_NEWS, W_COMMUTE = 0.30, 0.25, 0.20, 0.15, 0.10
...
class RichgoCortexEngine:
    ...
    External interface unchanged: engine.analyze(danji_id)
    """
    ...
    @staticmethod
    def compute_jeonse_ratio(mean_jeonse: float, mean_meme: float, is_dong_avg: bool = False) -> tuple:
        if not mean_meme or mean_meme == 0:
            return 0.0, 0.0
        return round(mean_jeonse / mean_meme, 4), (0.15 if is_dong_avg else 0.0)
    ...
    @staticmethod
    def compute_s_alpha(
        jeonse_ratio: float, pir: float, pir_5yr_avg: float,
        supply_score: float, sentiment_score: float,
        ...
        uv = pir_5yr_avg * PIR_UNDERVALUE_RATIO
        ov = pir_5yr_avg * 1.5
        if pir <= uv:
            s_pir = 100.0
        elif pir >= ov:
        ...
        base = W_SUPPLY*supply_score + W_PIR*s_pir + W_JEONSE*s_jeonse + W_NEWS*s_news + W_COMMUTE*commute_score
        if is_chobuma:
    ...
    def compute_confidence(deductions: list) -> tuple:
        ...
        return confidence, ("High" if confidence >= 85 else "Medium" if confidence >= 60 else "Low")

    def analyze(self, danji_id: str, news_texts: Optional[list] = None, commute_score: float = 50.0) -> dict:
        deductions = []
        ...
        latest = prices[0]
        jeonse_ratio, d_jeonse = self.compute_jeonse_ratio(latest["mean_jeonse_price"], latest["mean_meme_price"])
        ...
        pir_relative_index, pir_band_adj, pir_band_label = self.temporal.compute_adjustment(pir_val, pir_5yr_avg)
        region_prices = self._client.fetch_region_price(sgg, months=13)
        supply_score_final, d_supply, spillover_detail = self.spatial.compute_with_spillover(sgg, region_prices)
        ...
        s_alpha_raw = self.compute_s_alpha(
            jeonse_ratio=jeonse_ratio, pir=pir_val, pir_5yr_avg=pir_5yr_avg,
            supply_score=supply_score_final, sentiment_score=sentiment_score,
        ...
        return {
            "danji_id": danji_id, "danji_name": info["danji"],
            "sd": sd, "sgg": sgg, "emd": info["emd"],
            "analysis_date": datetime.now().strftime("%Y-%m-%d"),
            "s_alpha": s_alpha, "s_alpha_before_band": round(s_alpha_raw, 2),
            ...
            "jeonse_ratio": jeonse_ratio, "jeonse_floor": jeonse_floor,
            "jeonse_safety_ok": jeonse_ratio >= jeonse_floor,
            "pir": pir_val, "pir_5yr_avg": pir_5yr_avg,
            "pir_band_fallback": pir_band_fallback,
            "pir_relative_index": pir_relative_index, "pir_band_label": pir_band_label,
            "pir_undervalue_ok": pir_val <= pir_5yr_avg * PIR_UNDERVALUE_RATIO,
            "supply_score_raw": supply_score_raw, "supply_score_final": supply_score_final,
            "spillover_detail": spillover_detail,
            "sentiment_score": sentiment_score,
            "living_score": info.get("living_score"), "is_chobuma": is_chobuma,
