...
def _personalized_fallback(
    ...
    return {
        "score": score,
        "advisor_comment": (
            f"✅ 회원님의 가중치를 반영한 가중합(Weighted Sum) 기준 맞춤 점수는 {score}점입니다.\n"
            f"💡 가장 높은 기여 항목({top_domain})이 이 단지의 핵심 경쟁력으로 작용합니다.\n"
            f"📌 회원님께는 우선순위 기준에 부합하는 단지입니다."
        ),
        "matching_verdict": (
            f"✅ 회원님의 취향과 {match_pct}% 일치하는 단지입니다.\n"
            f"💡 회원님이 우려하시는 인프라 부족 요소가 전혀 없는 클린 단지입니다."
        ),
    }
